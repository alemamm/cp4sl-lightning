...
class KuramotoDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        self.prefix = "{seed}_{n_clusters}_{cluster_size}".format(
            seed=self.hparams.seed, n_clusters=self.hparams.n_clusters, cluster_size=self.hparams.cluster_size)
    ...
    def create_adjacency_matrix(self):
        ...
        for n in range(0, self.hparams.n_clusters):
            cluster_blocks.append(torch.ones([self.hparams.cluster_size, self.hparams.cluster_size]))
    ...
    def create_samples(self):
        ...
        nat_freqs = np.random.normal(loc=10, scale=1.0, size=self.hparams.n_clusters * self.hparams.cluster_size)
        model = Kuramoto(coupling=10, dt=0.001, T=self.hparams.n_timesteps*0.001, n_nodes=len(single_sample_adj.numpy()), natfreqs=nat_freqs)

        act_mats_train = []
        for i in tqdm(range(0, self.hparams.n_train)):
            act_mat = model.run(adj_mat=single_sample_adj.numpy(), angles_vec=np.random.uniform(0, 2*np.pi, size=self.hparams.n_clusters * self.hparams.cluster_size))
    ...
    def save_data(self, single_sample_adj, features_train, features_val, features_test):
        # features
        torch.save(torch.sin(features_train).clone(), osp.join(self.dataset_path, "features_train.pt"))
        torch.save(torch.sin(features_val).clone(), osp.join(self.dataset_path, "features_val.pt"))
        torch.save(torch.sin(features_test).clone(), osp.join(self.dataset_path, "features_test.pt"))
        # adj_original
